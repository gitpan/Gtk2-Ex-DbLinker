...
sub new {
   ...
   my $self = {
    ...
    my $builder = Gtk2::Builder->new();
     $builder->add_from_file($self->{gladefolder} . "/langues.bld")   or die "Couldn't read  langues.bld";
	...
	my $lstdata = Gtk2::Ex::DbLinker::DbiDataManager->new({
			...
			sql => { 
				...
				from => "langues inner join speaks on langues.langid = speaks.langid", 
				where=> "speaks.countryid =?", bind_values=>[0], },	
		});
	...
	print "on_countryid_changed: ", $value, "\n";
        $self->{langues}->get_data_manager->query({where =>"speaks.countryid=?", bind_values=>[$value]}) ;
	$self->{langues}->update;
}
